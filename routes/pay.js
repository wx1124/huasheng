			...
			rsjson = JSON.parse(JSON.stringify(rs[0]));
	      	...
	      	if(ii==len){
	      		...
	      		for(shopid in shopObj){
	      			...
	      			sequelize.query(sqlorder,{replacements: [shopObj[shopid].total,loginbean.id,shopid],type: sequelize.QueryTypes.INSERT}).then(function(rs){
	      				console.log(rs); //rs是sequelize返回刚插入的流水id
	      				kk++;
	      				if(kk==shopnum){
	      					res.send('订单插入');
	      				}
